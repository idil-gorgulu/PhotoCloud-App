...
/**
	 * A new instance of the Discover class is constructed.
	 * @param app the PhotoCloudApp instance
	 * @param user the User currently logged in
	 */
		public Discover(PhotoCloudApp app, User user) {
			this.app = app;
			this.user = user;
		...
		public void addPublicPosts() {
			for (int i = 0;i < app.getAll_posts().size(); i++) {
				if (app.getAll_posts().get(i).getIs_public()){
					...
					//content.setPreferredSize(new Dimension(440, 320 + app.getAll_posts().size()*280));
		...
		public JPanel PostPanel(User u, Post post) {
			...
			user_button.setBounds(1, 1, 210, 30);
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == my_profile) {
				...
				MyProfile profile = new MyProfile(app,discover.getUser());
				app.setUser(user);
			}
